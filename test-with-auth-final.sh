...
echo "🔐 Testing with Authentication - Final Check"
echo "============================================="
...
if [ ! -z "$TOKEN" ]; then
    ...
    DOCS_RESPONSE=$(curl -s -X GET "$BASE_URL/docs" \
      -H "Authorization: Bearer $TOKEN")
    echo "Documentation Status: $(echo "$DOCS_RESPONSE" | tail -c 4)"
    echo "Documentation Response: $(echo "$DOCS_RESPONSE" | head -c 200)..."
    
    # Test admin endpoint
    ADMIN_RESPONSE=$(curl -s -X GET "$BASE_URL/admin/metrics" \
      -H "Authorization: Bearer $TOKEN")
    echo "Admin Metrics Status: $(echo "$ADMIN_RESPONSE" | tail -c 4)"
    echo "Admin Metrics Response: $(echo "$ADMIN_RESPONSE" | head -c 200)..."
    
    # Test API client endpoint
    API_CLIENT_RESPONSE=$(curl -s -X GET "$BASE_URL/api-client/environments" \
      -H "Authorization: Bearer $TOKEN")
    echo "API Client Status: $(echo "$API_CLIENT_RESPONSE" | tail -c 4)"
    echo "API Client Response: $(echo "$API_CLIENT_RESPONSE" | head -c 200)..."
